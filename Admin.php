...
class WPCF7SAdmin {
	...
	public function __construct() {
		...
		add_action( 'pre_get_posts', [ $this, 'admin_posts' ] );
		add_action( 'pre_get_posts', [ $this, 'set_post_order' ] );

		add_action( 'wp', [ $this, 'export_request' ] );

		add_filter( 'page_row_actions', [ $this, 'action_row' ], 25, 2 );
		add_action( 'admin_enqueue_scripts', [ $this, 'scripts' ] );

		add_filter( 'views_edit-wpcf7s', [ $this, 'views' ], 999 );

		add_filter( 'gettext', [ $this, 'custom_status' ], 20, 2 );
	}
	...
	public function action_row( $actions, $post ) {
		...
		if ( 'wpcf7s' === $post_type ) {
			...
			$actions = array_merge( [ 'aview' => '<a href="' . get_edit_post_link( $post->ID ) . '">' . __( 'View', 'contact-form-submissions' ) . '</a>' ], $actions );
		}
		return $actions;
	}
	...
	public function meta_boxes() {
		...
		add_meta_box( 'wpcf7s_mail', __( 'Mail', 'contact-form-submissions' ), [ $this, 'mail_meta_box' ], 'wpcf7s', 'normal' );
		add_meta_box( 'wpcf7s_posted', __( 'Posted', 'contact-form-submissions' ), [ $this, 'posted_meta_box' ], 'wpcf7s', 'normal' );

		add_meta_box( 'wpcf7s_actions', __( 'Overview', 'contact-form-submissions' ), [ $this, 'actions_meta_box' ], 'wpcf7s', 'side' );
		remove_meta_box( 'submitdiv', 'wpcf7s', 'side' );

		// only show the meta box if the post has files
		$files = $this->get_mail_files( $post_id );
		if ( ! empty( $files ) ) {
			add_meta_box( 'wpcf7s_files', __( 'Files', 'contact-form-submissions' ), [ $this, 'files_meta_box' ], 'wpcf7s', 'normal' );
		}
	}
	...
	public function export_request() {
		if ( isset( $_GET['wpcf7s-export'] ) && ! empty( $_GET['wpcf7s-export'] ) && is_admin() ) {
			...
			// add default columns
			$pretty_columns = $columns = array_merge( [ 'id', 'submission_date', 'form_name' ], $columns );
			foreach ( $pretty_columns as $key => $column ) {
				// remove the plugin's prepended keys
				$pretty_columns[ $key ] = str_replace( [ 'wpcf7s_posted-', 'wpcf7s_file-' ], '', $column );
			}
